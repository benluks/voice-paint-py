...
std::vector<double> Resampler::process(const std::vector<double>& inDouble)
{
    std::vector<float> in(inDouble.begin(), inDouble.end());
    std::vector<float> out(getExpectedOutLength(in.size()));

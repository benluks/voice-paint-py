...
static std::vector<double> downsampleSignal(const std::vector<double>& s, const double Fs, const double Fds);
...
std::vector<double> downsampleSignal(const std::vector<double>& s, const double Fs, const double Fds)
{
    size_t olen = s.size() * Fds / Fs + 0.5;
    
    std::vector<float> sFloat(s.begin(), s.end());
    std::vector<float> outFloat(olen);
    ...
    return std::vector<double>(outFloat.begin(), outFloat.end());
}
